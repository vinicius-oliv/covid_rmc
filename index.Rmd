...
ifelse(Dia == "01", Dia <- "1o", ifelse(as.numeric(Dia) < 10, Dia <- as.character(as.numeric(Dia)), Dia <- dia))
Mes <- meses[as.integer(mes) - 2]

d <- dmy(paste0(dia, mes,"2020"))

dia_ext = c("domingo", "segunda", "terca", "quarta", "quinta", "sexta", "sabado")

dia_ext <- dia_ext[wday(d)]
...
#url <- paste0("https://g1.globo.com/sp/campinas-regiao/noticia/2020/", mes, "/", dia, "/coronavirus-prefeituras-da-regiao-de-campinas-confirmam-novos-casos-", nest ,"-",dia_ext,"-",Dia,"-de-",Mes,".ghtml")

url <- "https://g1.globo.com/sp/campinas-regiao/noticia/2020/05/04/coronavirus-prefeituras-da-regiao-de-campinas-confirmam-mortes-e-casos-nesta-segunda-4-de-maio.ghtml"
...
if(length(cidades) == 0){
  ...
  texto <- url %>% read_html() %>% html_nodes("ol.content-ordered-list") %>% html_text()
...
x = tm_shape(ultimos_casos) + tm_fill(col = "Total", style = "jenks", palette = pal, popup.vars = c("Casos: " = "Total")) + tm_borders(col = NA, lwd = 1.5, lty = "solid", alpha = NA, group = NA) + tm_layout(title = "Clique na cidade") + tm_text("name_muni", size = .8)
tmap_mode("view") 
x
...
p<- ggplot(data = k, aes(x = Data, y = Total, color = Cidade)) +
  ...
  scale_y_continuous(breaks=seq(0, max(casos$Total) + 30, 30),
...
n<- ggplot(data = totalData, aes(x = Data, y = Total)) +
  ...
  scale_y_continuous(breaks=seq(0, max(totalData$Total) + ceiling(max(totalData$Total)/10), 50),
