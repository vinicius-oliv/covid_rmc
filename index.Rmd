...
url <- paste0("https://www.google.com/search?q=coronavirus+", dia, "+de+", Mes, "+campinas")
...
for(i in 1:length(texto)){
  ...
  if(str_detect(texto[i], "Campinas:|Campinas-")& 
     str_detect(texto[i], "Valinhos:|Valinhos-")){
        cidades <- texto[i]
  }
...
p<- casos %>% group_by(Cidade) %>% filter(Total != 0) %>% 
  ...
  scale_y_continuous(breaks=seq(0, max(casos$Total) + 30, 50),
...
n<- ggplot(data = totalData, aes(x = Data, y = Total)) +
  ...
  scale_y_continuous(breaks=seq(0, max(totalData$Total) + ceiling(max(totalData$Total)/10), 200),
