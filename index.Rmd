...
p<- casos %>% group_by(Cidade) %>% filter(Total != 0) %>% 
  ...
  scale_y_continuous(breaks=seq(0, log(max(casos$Total)) + 1, 0.5),
                     limits = c(0, log(max(casos$Total)) + 1.5),
                     ) +
...
ggplotly(p) %>% plotly::config(displayModeBar = F) %>% layout(legend=list(orientation = "h", x = 0, y = -0.2), annotations = list(
 list(x = 0 , y = 1.085, text = "(Dois cliques para isolar uma cidade)", showarrow = F, xref='paper', yref='paper')))
```
...
n<- ggplot(data = totalData, aes(x = Data, y = Total)) +
  ...
  scale_y_continuous(breaks=seq(0, max(totalData$Total) + ceiling(max(totalData$Total)/10), 500),
